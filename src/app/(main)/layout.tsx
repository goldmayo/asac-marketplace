...
<section className="relative h-screen">
  ...
  <nav className="sticky bottom-0 h-14 bg-white border border-t-gray-300 text-2xl flex items-center justify-around">
    ...
    <Link className={`${segment === '(category)' ? 'text-brand-primary-500' : 'text-black'}`} href={"/recommendations"}>
        <CiMenuBurger></CiMenuBurger>
    </Link>
    <Link className={`${segment === '(search)' ? 'text-brand-primary-500' : 'text-black'}`} href={"/recommendations"}>
        <CiSearch></CiSearch>
    </Link>
    <Link className={`${segment === '(myPage)' ? 'text-brand-primary-500' : 'text-black'}`} href={"/recommendations"}>
        <CiUser></CiUser>
    </Link>
  </nav>
</section>
